...
def generate_paths(args):
   ...
   for months, days in zip(args.months, args.days) :
      ...
      if("-" in days):
         ...
         index1 = days_full.index(parts[0])
         index2 = days_full.index(parts[1]) + 1
      else :
         index1 = days_full.index(days)
         index2 = index1 + 1
      
      for i in list(days_full.keys())[index1 : index2]:
            if(args.read_or_write == "t"):
               paths.append(os.path.join(months,i,get_time(days_index)))
            else :
               paths.append(os.path.join(months,i,get_time(days_index),"plik.csv"))
